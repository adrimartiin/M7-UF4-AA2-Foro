...
if (!isset($_SESSION['usuario'])) {
    ...
    echo 'Error! Debes tener un usuario para guardar las preguntas';
} else {
    $estado_pregunta = 'guardada';
    $id_usuario = $_SESSION['usuario'];
    try {
        //update para que el estado de la pregunta cambie a guardada
        $sql = "UPDATE tbl_preguntas SET estado_preguntas = :estado_preguntas WHERE id_usuario = :id_usuario";
        $stmt = $conexion->prepare($sql);
        $stmt->bindParam(':estado_preguntas', $estado_pregunta);
        ...
        <div class="container">
            ...
            <div class="preguntas-contenedor">
                ...
                <div class="barra-derecha">
                    <?php
                    try {
                        $stmt = $conexion->query("SELECT id_preguntas, titulo_preguntas, texto_preguntas, tbl_preguntas.id_usuario, nombre_usuario, estado_preguntas
                        FROM tbl_preguntas INNER JOIN tbl_usuarios ON tbl_preguntas.id_usuario = tbl_usuarios.id_usuario WHERE estado_preguntas = :estado_preguntas");

                        $guardar_pregs = $stmt->fetchAll(PDO::FETCH_ASSOC);

                        // Si no hay preguntas muestra mensaje de que no hay, si hay muestra preguntas
                        if (empty($guardar_pregs)) {
                            echo "No hay preguntas guardadas en este momento";
                        } else {
                            echo '<ul style="list-style-type: none; padding: 0;">';
                            foreach ($guardar_pregs as $guarda_pregunta) {
                                echo '<li class="pregunta-container" style="margin-bottom: 20px; position: relative;">';
                                echo '<form action="gi.php" method="post">';
